...
// Class-list joiners the tracer looks through: `cx` from '@linaria/core' and
// anything imported from these packages (`import cn from 'clsx'`), under any
// local name. A function of the same name from anywhere else (same file,
// component local, other module) is a helper and is followed as one.
const CX_SOURCES = new Set(['clsx', 'classnames']);
...
/**
 * Local names bound to the processor tags in a program, following import
 * aliases (`import { css as linariaCss }`): Map(localName -> 'styled' | 'css').
 * Shared with static-chains so both modules agree on what a tag is.
 */
const processorLocalsCache = new WeakMap();
const collectProcessorLocals = (program) => {
  const cached = processorLocalsCache.get(program);
  ...
  processorLocalsCache.set(program, locals);
  return locals;
};

const isStyledTag = (tag, locals) => {
  const t = unwrapTS(tag);
  if (!t) return false;
  // styled.div``  |  styled('div')``  |  styled(X)``
  if (t.type === 'MemberExpression' || t.type === 'StaticMemberExpression') {
    return (
      t.object?.type === 'Identifier' && locals.get(t.object.name) === 'styled'
    );
  }
  if (t.type === 'CallExpression') {
    return (
      t.callee?.type === 'Identifier' && locals.get(t.callee.name) === 'styled'
    );
  }
  return false;
};

const isCssTag = (tag, locals) => {
  const t = unwrapTS(tag);
  return t?.type === 'Identifier' && locals.get(t.name) === 'css';
};
...
/**
 * Finds what `name` is bound to at module level of `program`. Returns a
 * descriptor:
 *
 * - { kind: 'function', node } (plus `closure` when the function is what a local
 *   factory call returns: Map(param name -> argument expression))
 * - { kind: 'styled', node } / { kind: 'css', node } (Linaria-tagged
 *   declarations)
  ...
  for (const rawStatement of program.body) {
    ...
    if (statement.type === 'ExportDefaultDeclaration' && name === 'default') {
      ...
      if (declaration?.type === 'Identifier') {
        return withAliasName(
          findModuleBinding(program, declaration.name),
          declaration.name,
        );
      }
      ...
      if (statement.type === 'ExportNamedDeclaration') {
        ...
        for (const spec of statement.specifiers ?? []) {
          ...
          if (
            ...
            return withAliasName(
              findModuleBinding(program, spec.local.name),
              spec.local.name,
            );
          }
        }
    ...
    if (statement.type === 'VariableDeclaration') {
      for (const decl of statement.declarations) {
        ...
        if (init.type === 'TaggedTemplateExpression') {
          ...
          if (isStyledTag(init.tag, locals))
            return { kind: 'styled', node: init };
          if (isCssTag(init.tag, locals)) return { kind: 'css', node: init };
        ...
        if (init.type === 'CallExpression') {
          ...
          if (
            ...
            if (arg?.type === 'Identifier')
              return withAliasName(
                findModuleBinding(program, arg.name),
                arg.name,
              );
          }
        ...
        if (init.type === 'Identifier')
          return withAliasName(
            findModuleBinding(program, init.name),
            init.name,
          );
        return { kind: 'expression', node: init };
      }
...
// realpath so pnpm workspace symlinks (node_modules/@zdf/*) resolve back into
 ...
 * following imports and re-exports. Returns { filename, name, binding } with
 * binding from findModuleBinding, or throws.
 */
...
const staticStringOf = (filename, name, depth = 0) => {
  ...
  return staticStringOfExpression(
    resolved.filename,
  ...
  if (n.type === 'TemplateLiteral') {
    ...
    for (let i = 0; i < n.quasis.length; i += 1) {
      ...
      if (i < n.expressions.length) {
        const part = staticStringOfExpression(
          filename,
          n.expressions[i],
          depth + 1,
        );
...
// True when `callee` is a class-list joiner (cx / clsx / classNames or an
// alias of one). A function-local binding under that name shadows the joiner,
// and a same-file module binding is a helper, followed by classifyHelperCall.
const isCxCallee = (callee, ctx) => {
  const c = unwrapTS(callee);
  if (c?.type !== 'Identifier') return false;
  const { scope, filename } = ctx;
  if (
    scope.locals.has(c.name) ||
    scope.values.has(c.name) ||
    scope.propNames.has(c.name) ||
    scope.hookResults.has(c.name) ||
    scope.aliases.has(c.name)
  ) {
    return false;
  }
  const binding = findModuleBinding(parseFile(filename).program, c.name);
  if (binding?.kind !== 'import') return false;
  if (binding.source === '@linaria/core') return binding.imported === 'cx';
  return CX_SOURCES.has(binding.source);
};
...
const isHookCall = (node) => {
  ...
  if (
    ...
    return (
      callee.property?.type === 'Identifier' && isHookName(callee.property.name)
    );
  }
  return false;
};
...
// The value an assignment or update leaves in its target. `x = v` and the
// logical forms (`x ||= v`, `x ??= v`, `x &&= v`) leave `v` or the previous
// value, which the local's list already holds. Arithmetic compounds
// (`x += '-y'`) and updates (`x++`) leave `x + '-y'` / `x + 1`, modelled as
// the binary expression so a class position reading it fails like a
// written-out `+` would. Synthetic nodes are cached per mutation: the scope
// walk runs to a fixed point and must not push a fresh node each round.
const LOGICAL_ASSIGNMENT_OPERATORS = new Set(['||=', '??=', '&&=']);
const compoundValues = new WeakMap();
const assignedValue = (mutation) => {
  const { operator } = mutation;
  const isUpdate = mutation.type === 'UpdateExpression';
  if (
    !isUpdate &&
    (operator === '=' || LOGICAL_ASSIGNMENT_OPERATORS.has(operator))
  ) {
    return mutation.right;
  }
  let synthetic = compoundValues.get(mutation);
  if (!synthetic) {
    synthetic = {
      type: 'BinaryExpression',
      operator: isUpdate ? operator[0] : operator.slice(0, -1),
      left: isUpdate ? mutation.argument : mutation.left,
      right: isUpdate ? { type: 'NumericLiteral', value: 1 } : mutation.right,
      start: mutation.start,
      end: mutation.end,
      range: mutation.range,
    };
    compoundValues.set(mutation, synthetic);
  }
  return synthetic;
};

// Target identifier of an assignment / update to a plain identifier, or null.
const mutatedName = (node) => {
 ...
 *
 * `inputs` describes what each parameter receives, in call order: { kind:
 * 'props', tracked: string[] } an object whose keys named in `tracked` carry
 * the traced value (component props; `tracked` is ['className'] at the top
 * level) { kind: 'tracked' } the traced value itself { kind: 'value', refs } a
 * provable class value (helper args) { kind: 'unknown' } anything else
 */
...
const buildScope = (fn, inputs, siteProps, parent = null, closure = null) => {
  ...
  const bindPattern = (pattern, input) => {
    ...
    if (input.kind === 'tracked') {
      if (p.type === 'Identifier') scope.tracked.add(p.name);
        ...
        scope.failures.push(
          'traced value destructured into a non-identifier pattern',
        );
      return;
    }
    if (input.kind === 'value') {
    ...
    if (p.type === 'ObjectPattern')
      destructureCarrier(p, new Set(input.tracked));
  };
  ...
  const destructureCarrier = (objectPattern, trackedKeys) => {
    ...
    for (const prop of objectPattern.properties ?? []) {
      ...
      if (value?.type !== 'Identifier') {
        if (trackedKeys.has(key)) {
          scope.failures.push(
            `prop '${key}' destructured into a non-identifier pattern`,
          );
        }
        continue;
      }
    ...
    for (const prop of objectPattern.properties ?? []) {
      ...
      const remaining = new Set(
        [...trackedKeys].filter((key) => !seenKeys.has(key)),
      );
  ...
  while (changed) {
    ...
    walk(fn.body, (node) => {
      ...
      const mutated = mutatedName(node);
      if (mutated !== null) {
        if (scope.locals.has(mutated)) {
          const list = scope.locals.get(mutated);
          const value = assignedValue(node);
          if (!list.includes(value)) list.push(value);
        }
        return;
      }
      if (node.type !== 'VariableDeclarator' || handledDeclarators.has(node))
        return;
      ...
      if (node.id?.type === 'ObjectPattern') {
        if (init?.type === 'Identifier' && scope.carriers.has(init.name)) {
          ...
          destructureCarrier(
            node.id,
            scope.carrierTracked.get(init.name) ?? new Set(),
          );
          changed = true;
          return;
        }
      ...
      if (init?.type === 'ObjectExpression') {
        ...
        // tracked keys are re-set. Last write wins, so properties are taken
        // in source order: `{ className: 'x', ...props }` still carries
        // props.className, `{ ...props, className: 'x' }` does not. (A re-set
        // that reads the traced value is a leak the accounting pass reports.)
        const properties = init.properties ?? [];
        const spreadsCarrier = properties.some((prop) => {
          if (prop.type !== 'SpreadElement') return false;
          const arg = unwrapTS(prop.argument);
          return arg?.type === 'Identifier' && scope.carriers.has(arg.name);
        });
        if (spreadsCarrier) {
          ...
          for (const prop of properties) {
            if (prop.type === 'SpreadElement') {
              ...
              if (arg?.type === 'Identifier' && scope.carriers.has(arg.name)) {
                for (const key of scope.carrierTracked.get(arg.name) ?? [])
                  trackedKeys.add(key);
              }
              continue;
            }
...
const moduleObjectLiteral = (filename, name) => {
  ...
  return node?.type === 'ObjectExpression'
    ? { filename: resolved.filename, node }
    : null;
};

/**
 * The properties of a module-level object literal a member access can select,
 * respecting last-write-wins. A spread may bring in any key, so it is
 * unprovable when it can still decide the result: any spread for a computed
 * access (`map[kind]`), a spread after the last literal `key` property (or
 * without one at all) for a static access (`map.kind`).
 */
const literalObjectProperties = (literalObject, memberNode, objectName) => {
  const properties = literalObject.node.properties ?? [];
  const hasSpread = properties.some((p) => p.type === 'SpreadElement');
  if (memberNode.computed) {
    if (hasSpread) throw new Unprovable(`spread inside '${objectName}'`);
    return properties;
  }
  const key = memberNode.property?.name;
  let selected = null;
  let spreadAfter = false;
  for (const prop of properties) {
    if (prop.type === 'SpreadElement') {
      spreadAfter = true;
    } else if (propertyKeyName(prop) === key) {
      selected = prop;
      spreadAfter = false;
    }
  }
  if (selected && !spreadAfter) return [selected];
  if (hasSpread) {
    throw new Unprovable(
      `'${objectName}.${key}' may be overridden by a spread inside '${objectName}'`,
    );
  }
  throw new Unprovable(`'${objectName}.${key}' is not a literal property`);
};
  ...
  switch (node.type) {
    case 'Identifier': {
      ...
      if (scope.values.has(name))
        return { tracked: false, refs: [...scope.values.get(name)] };
      if (scope.locals.has(name)) {
        ...
        if (inits.length === 0) {
          throw new Unprovable(
            `local '${name}' has no statically visible value`,
          );
        }
        for (const init of inits)
          result = mergeClasses(result, classifyClassExpr(init, ctx, next));
        return result;
      }
      if (scope.propNames.has(name)) {
        // consumer-controlled prop: use the call-site value when the traced
        // tree binds it, else the declared default (what the element carries
        // whenever the consumer passes nothing), else out of contract
        ...
        if (scope.propDefaults.has(name)) {
      ...
      if (scope.hookResults.has(name)) {
        throw new Unprovable(
          `'${name}' comes from a hook result and may carry component classes`,
        );
      }
      const { program } = parseFile(filename);
      if (findModuleBinding(program, name)) {
        return {
          tracked: false,
          refs: [{ kind: 'css-ref', file: filename, localName: name }],
        };
      }
    ...
    case 'LogicalExpression': {
      if (node.operator === '&&')
        return classifyClassExpr(node.right, ctx, visiting);
      return mergeClasses(
    ...
    case 'ArrayExpression': {
      ...
      for (const element of node.elements ?? []) {
        ...
        if (element.type === 'SpreadElement')
          throw new Unprovable('spread inside a class array');
        result = mergeClasses(
          result,
          classifyClassExpr(element, ctx, visiting),
        );
      }
      return result;
    }
    case 'ObjectExpression': {
      ...
      for (const prop of node.properties ?? []) {
        if (prop.type === 'SpreadElement')
          throw new Unprovable('spread inside a class object');
        if (prop.computed) {
          result = mergeClasses(
            result,
            classifyClassExpr(prop.key, ctx, visiting),
          );
        } else {
          const key = propertyKeyName(prop);
          if (key !== null) {
    ...
    case 'CallExpression': {
      if (isCxCallee(node.callee, ctx))
        return cxLikeArgs(node.arguments, ctx, visiting);
    ...
    case 'ComputedMemberExpression': {
      ...
      if (object?.type === 'Identifier' && scope.carriers.has(object.name)) {
        if (node.computed)
          throw new Unprovable(
            `computed access on props object '${object.name}'`,
          );
      ...
      if (object?.type === 'Identifier') {
        ...
        if (literalObject) {
          ...
          // variants.primary: that value; variants[variant]: any value may apply
          let result = emptyClasses();
          for (const prop of literalObjectProperties(
            literalObject,
            node,
            object.name,
          )) {
            result = mergeClasses(
              result,
              classifyClassExpr(prop.value, objectCtx, visiting),
            );
          }
          return result;
        }
      ...
      throw new Unprovable(
        `member expression '${ctx.code.slice(node.start ?? node.range?.[0], node.end ?? node.range?.[1])}' in class position`,
      );
    }
...
const classifyHelperCall = (call, ctx, visiting) => {
  ...
  if (localInits) {
    ...
    if (fns.length !== 1 || fns.length !== localInits.length) {
      throw new Unprovable(
        `class value passed to local '${callee.name}', which is not a single function`,
      );
    }
    helperFn = fns[0];
    ...
    try {
      ...
      throw new Unprovable(
        `class value passed to function '${callee.name}' (${e.message})`,
      );
    }
  ...
  const inputs = (call.arguments ?? []).map((arg) => {
    if (arg.type === 'SpreadElement')
      throw new Unprovable(`spread argument to '${callee.name}'`);
    const a = unwrapTS(arg);
    if (a?.type === 'Identifier' && ctx.scope.carriers.has(a.name)) {
      return {
        kind: 'props',
        tracked: [...(ctx.scope.carrierTracked.get(a.name) ?? [])],
      };
    }
    const classes = classifyClassExpr(arg, ctx, visiting);
    if (classes.tracked && classes.refs.length === 0)
      return { kind: 'tracked' };
    if (classes.tracked) {
  ...
  if (helperScope.failures.length > 0) {
    throw new Unprovable(
      `helper '${callee.name}': ${helperScope.failures.join('; ')}`,
    );
  }
  const helperCtx = {
  ...
  if (returns.length === 0) {
    throw new Unprovable(
      `helper '${callee.name}' has no statically visible return value`,
    );
  }
  let result = emptyClasses();
  try {
    ...
    if (e instanceof Unprovable)
      throw new Unprovable(`helper '${callee.name}': ${e.message}`);
    throw e;
  }
  return result;
};
...
const resolveElementExpr = (expression, ctx, visiting = new Set()) => {
  ...
  switch (node.type) {
    case 'Identifier': {
      const { name } = node;
      if (scope.propNames.has(name) || scope.carriers.has(name))
        return [{ type: 'runtime' }];
      ...
      if (scope.locals.has(name)) {
        ...
        if (inits.length === 0)
          throw new Unprovable(
            `element '${name}' has no statically visible value`,
          );
    ...
    case 'LogicalExpression':
      if (node.operator === '&&')
        return resolveElementExpr(node.right, ctx, visiting);
      return [
      ...
      if (object?.type === 'Identifier') {
        ...
        if (literalObject) {
          ...
          return literalObjectProperties(
            ...
            node,
            object.name,
          ).flatMap((p) => resolveElementExpr(p.value, objectCtx, visiting));
        }
      }
...
const resolveJsxElementName = (element, ctx) => {
  ...
  if (nameNode.type !== 'JSXIdentifier') {
    throw new Unprovable(
      'className forwarded to a JSX member expression element',
    );
  }
  if (/^[a-z]/.test(nameNode.name))
    return [{ type: 'dom', name: nameNode.name }];
...
const attributeName = (attr) =>
  attr.name?.name ?? attr.name?.name?.name ?? null;

/**
 * Traces the given component function. Returns { targets, failures } where each
 * target is { kind: 'dom' | 'runtime', refs } { kind: 'component', name, refs,
 * siteProps: Map(prop -> { tracked, refs }) }
 */
const traceFunction = (fn, code, filename, inputs, siteProps, closure) => {
  ...
  // The scope model is flat across nested functions (callback locals are
  // followed like the component's own). A nested declaration re-using a name
  // bound outside it would be read as that binding everywhere, including
  // outside the callback. Refuse instead of guessing when the name takes part
  // in the trace: the traced value, a carrier, a module binding such as a
  // component or `cx`, or a local / param that feeds a class or element
  // expression (decided once those are known, below).
  const { program } = parseFile(filename);
  const declaredNames = (node, out) => {
    switch (node.type) {
      case 'VariableDeclarator':
        return patternIdentifiers(node.id, out);
      case 'FunctionDeclaration':
      case 'ClassDeclaration':
        if (node.id?.type === 'Identifier') out.push(node.id.name);
        return out;
      case 'CatchClause':
        return patternIdentifiers(node.param, out);
      default:
        if (isFunctionNode(node)) {
          for (const param of node.params ?? []) patternIdentifiers(param, out);
        }
        return out;
    }
  };
  const outerDeclared = new Set(declaredNames(fn, []));
  const nestedDeclared = new Set();
  walk(fn.body, (node, ancestors) => {
    const inNested = ancestors.some((a) => a !== fn && isFunctionNode(a));
    const target =
      inNested || (isFunctionNode(node) && node !== fn)
        ? nestedDeclared
        : outerDeclared;
    for (const name of declaredNames(node, [])) target.add(name);
    return undefined;
  });

  const isTrackedReference = (node) =>
    node.type === 'Identifier' &&
    (scope.tracked.has(node.name) || scope.carriers.has(node.name));
    ...
    walk(expression, (node, ancestors) => {
      ...
      if (
        ...
        if (
          scope.locals
            .get(node.name)
            .some((init) => containsTracked(init, next))
        ) {
          found = true;
          return false;
        }
      ...
      if (
        isMember(parent) &&
        parent.object === node &&
        scope.carriers.has(node.name)
      ) {
        const trackedKeys = scope.carrierTracked.get(node.name) ?? new Set();
        if (!parent.computed && !trackedKeys.has(parent.property?.name))
          return undefined;
      }
  ...
  // True when `expression` reads local `name`, directly or through other
  // locals (`const a = flag; <X className={a} />`).
  const referencesLocal = (expression, name, visiting = new Set()) => {
    let found = false;
    walk(expression, (node, ancestors) => {
      if (found || node.type !== 'Identifier') return found ? false : undefined;
      const parent = ancestors[ancestors.length - 1];
      if (isMember(parent) && parent.property === node && !parent.computed)
        return undefined;
      if (
      ...
      if (scope.locals.has(node.name) && !visiting.has(node.name)) {
        const next = new Set(visiting).add(node.name);
        if (
          scope.locals
            .get(node.name)
            .some((init) => referencesLocal(init, name, next))
        ) {
          found = true;
          return false;
        }
  ...
  // locals bound to the result of an untraced call that received the traced
  // value: { names, viaCall }, checked for reaching an accounted expression
  const pendingCallResults = [];
  // expressions the trace reasons about, for the nested redeclaration check:
  // className values, spread arguments, element names of targets and the
  // other attributes handed to component targets (element aliases and class
  // values are locals too; a `value={x}` on a DOM node is not)
  const traceRelevant = [];
  ...
  walk(fn.body, (node) => {
    ...
    // Per prop, the attribute that finally sets it: JSX props are last-write-
    // wins, so `<X {...props} className="x" />` does not forward
    // props.className and `<X className="x" {...props} />` does. A carrier
    // spread sets exactly the keys it carries tracked.
    const propSources = new Map(); // prop -> { expression } | { spread: true }
    for (const attr of opening.attributes ?? []) {
      if (attr.type === 'JSXSpreadAttribute') {
        accounted.add(attr.argument);
        traceRelevant.push(attr.argument);
        const arg = unwrapTS(attr.argument);
        if (arg?.type === 'Identifier' && scope.carriers.has(arg.name)) {
          for (const key of scope.carrierTracked.get(arg.name) ?? [])
            propSources.set(key, { spread: true });
      ...
      if (expression) accounted.add(expression);
      propSources.set(name, { expression: expression ?? null });
    }
    const classSource = propSources.get('className');
    const classAttr = classSource?.expression ?? null;
    if (classAttr) traceRelevant.push(classAttr);
    const spreadTrackedProps = [...propSources.entries()]
      .filter(([, source]) => source.spread)
      .map(([key]) => key);
    const spreadsCarrier = spreadTrackedProps.length > 0;
    const siteAttrs = [...propSources.entries()]
      .filter(([name, source]) => name !== 'className' && source.expression)
      .map(([name, source]) => ({ name, expression: source.expression }));
    ...
    if (
      ...
      traceRelevant.push({ type: 'Identifier', name: opening.name.name });
      traceRelevant.push(...siteAttrs.map(({ expression }) => expression));
    }
  ...
  for (const name of nestedDeclared) {
    ...
    const shadowsRelevantLocal =
      ...
      traceRelevant.some((expression) => referencesLocal(expression, name));
    if (shadowsTrace || shadowsRelevantLocal) {
      failures.push(`'${name}' is redeclared inside a nested function`);
    }
  }
  ...
  walk(fn.body, (node, ancestors) => {
    ...
    if (
      ancestors.some(
        (a) => a.type === 'ObjectPattern' || a.type === 'ArrayPattern',
      )
    ) {
    ...
    const leak = (what) => {
      failures.push(
        viaCall ? `className passed to function '${viaCall}'` : what,
      );
    };
    for (let i = ancestors.length - 1; i >= 0; i -= 1) {
      ...
      if (ancestor.type === 'VariableDeclarator') {
        ...
        // already modelled. `const x = helper(props)` is only fine when x
        // does reach one (checked after the walk): a result that is merely
        // tested or dropped leaves the call as opaque as a bare statement.
        const direct =
          unwrapTS(ancestor.init) === unwrapTS(child) ||
          ancestor.id?.type === 'ObjectPattern';
        if (direct || viaCall) {
          if (viaCall) {
            pendingCallResults.push({
              names: patternIdentifiers(ancestor.id),
              viaCall,
            });
          }
          return undefined;
      ...
      if (
        ...
        if (scope.locals.has(ancestor.left.name)) {
          // like a declarator: a call result must still reach a class position
          if (viaCall) {
            pendingCallResults.push({ names: [ancestor.left.name], viaCall });
          }
          return undefined;
        }
      ...
      // Boolean positions consume the value without forwarding it, unless a
      // call sits in between (`if (consume(className))`): its result is
      // discarded like a bare statement's, so the call is a leak.
      const booleanPosition =
      ...
      if (booleanPosition) {
        if (viaCall) leak('className used in a boolean position');
        return undefined;
      }
      ...
      if (
        ...
        !isCxCallee(ancestor.callee, ctx)
      ) {
        ...
        if (scope.carriers.has(node.name) && isHookCall(ancestor))
          return undefined; // hooks do not render
        continue;
      }
  ...
  for (const { names, viaCall } of pendingCallResults) {
    const reaches = names.some((name) =>
      [...accounted].some((expression) => referencesLocal(expression, name)),
    );
    if (!reaches) failures.push(`className passed to function '${viaCall}'`);
  }
...
/**
 * Top-level entry: trace which class chains `styled(X)` has to beat, where X is
 * `name` bound in `filename` (typically via the consumer's import).
 *
 * Returns { status: 'traced', chains } or { status: 'unsupported', failures }.
 * Each chain entry is a list of refs to resolve to class names: { file,
 * localName, kind: 'styled' | 'css' | 'css-ref' | 'css-literal' }.
 *
 * Opts (used when recursing into a child component): trackedProps: string[]
 * props carrying the traced value (default ['className']) siteProps: Map(prop
 * -> { tracked, refs }) class values the call site binds
 */
const traceStyleTargets = (filename, name, depth = 0, opts = {}) => {
  ...
  if (binding.kind !== 'function') {
    return {
      status: 'unsupported',
      failures: [
        `'${name}' in ${resolved.filename} is not a traceable function (${binding.kind})`,
      ],
    };
  }
